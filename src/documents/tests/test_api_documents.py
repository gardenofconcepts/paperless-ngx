...
class TestDocumentApi(DirectoriesMixin, DocumentConsumeDelayMixin, APITestCase):
    ...
    def test_upload_with_multiple_custom_field_instances(self):
        """
        GIVEN: A document with a source file
        WHEN: Upload the document with multiple instances of the same custom field
        THEN: Metadata includes all instances correctly
        """
        self.consume_file_mock.return_value = celery.result.AsyncResult(
            id=str(uuid.uuid4()),
        )
        ...
        with (Path(__file__).parent / "samples" / "simple.pdf").open("rb") as f:
            response = self.client.post(
                "/api/documents/post_document/",
                {
                    ...
                    "custom_fields": json.dumps(
                        [
                            {"field": cf_string.id, "value": "first value"},
                            {"field": cf_string.id, "value": "second value"},
                            {"field": cf_string.id, "value": "third value"},
                        ],
                    ),
                },
            )
        ...
        self.assertEqual(
            overrides.custom_fields,
            [
                {"field_id": cf_string.id, "value": "first value"},
                {"field_id": cf_string.id, "value": "second value"},
                {"field_id": cf_string.id, "value": "third value"},
            ],
        )
